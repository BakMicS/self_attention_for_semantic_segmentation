...
def san19(pretrained=False):
    model = san(sa_type=0, layers=(3, 3, 4, 6, 3), kernels=[3, 3, 3, 3, 3], num_classes=1000)
    # model = san(sa_type=0, layers=(4, 6, 8, 16, 4), kernels=[3, 7, 7, 7, 7], num_classes=1000)  # san 38
    if pretrained:
        model = san(sa_type=0, layers=(3, 3, 4, 6, 3), kernels=[3, 7, 7, 7, 7], num_classes=1000)
        model = torch.nn.DataParallel(model.cuda())
        checkpoint = torch.load('./pretrained_models/san19_pairwise/model/model_best.pth')
        model.load_state_dict(checkpoint['state_dict'], strict=True)
        model = model.module
    return model


def san12(pretrained=False):
    model = san(sa_type=0, layers=(3, 3, 4, 1, 1), kernels=[3, 3, 3, 3, 3], num_classes=1000)
    if pretrained:
        model = san(sa_type=0, layers=(3, 3, 4, 1, 1), kernels=[3, 3, 3, 3, 3], num_classes=1000)
        model = torch.nn.DataParallel(model.cuda())
        checkpoint = torch.load('./pretrained_models/san19_pairwise/model/model_best.pth')
...
if __name__ == '__main__':
    net = san19().cuda() # san(sa_type=0, layers=(3, 4, 6, 8, 3), kernels=[3, 7, 7, 7, 7], num_classes=1000)
    print(net)
